...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">ระบบยื่นคำขอการนำเข้าผลิตภัณฑ์เพื่อใช้ในประเทศเป็นการเฉพาะคราว (21 ทวิ)</h3>

                    <div class="pull-right">

                      @can('add-'.str_slug('applicant-21bis'))
                          <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/esurv/applicant_21bis/create') }}">
                            <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                          </a>
                      @endcan

                      @can('delete-'.str_slug('applicant-21bis'))
                    ...
                    <hr>
										...
										{!! Form::model($filter, ['url' => '/esurv/applicant_21bis', 'method' => 'get', 'id' => 'myFilter']) !!}
                    ...
                    <div class="table-responsive">
                      ...
                      {!! Form::open(['url' => '/esurv/applicant_21bis/multiple', 'method' => 'delete', 'id' => 'myForm', 'class'=>'hide']) !!}

                      {!! Form::close() !!}

                      {!! Form::open(['url' => '/esurv/applicant_21bis/update-state', 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                      ...
                      {!! Form::close() !!}
                        ...
                        <table class="table table-borderless" id="myTable">
                            ...
                            <tbody>
                            @foreach($applicant_21bis as $key => $item)
                                <tr>
                                    {{-- <td>{{ $applicant_21bis->firstItem()+$key }}</td> --}}
                                    <td class="text-top">{{ $applicant_21bis->perPage()*($applicant_21bis->currentPage()-1)+$loop->iteration }}</td>
                                    ...
                                    <td class="text-center">
                                        {{-- @can('view-'.str_slug('applicant-21bis'))
                                            <a href="{{ url('/esurv/applicant_21bis/' . $item->id) }}"
                                               title="View Applicant21Bis" class="btn btn-info btn-xs">
                                    ...
                                    @if(in_array($item->state,['1','3']))
                                              @can('edit-'.str_slug('applicant-21bis'))
                                                  <a href="{{ url('/esurv/applicant_21bis/' . $item->id . '/edit') }}"
                                                    title="Edit Applicant21Bis" class="btn btn-primary btn-xs">
                                              ...
                                              @can('delete-'.str_slug('applicant-21bis'))
                                                  {!! Form::open([
                                                                  'method'=>'DELETE',
                                                                  'url' => ['/esurv/applicant_21bis', $item->id],
                                                                  'style' => 'display:inline'
                                                  ]) !!}
                                                  {!! Form::button('<i class="fa fa-trash-o" aria-hidden="true"></i>', array(
                                                          'type' => 'submit',
                                                          'class' => 'btn btn-danger btn-xs',
                                                          'title' => 'Delete Applicant21Bis',
                        ...
                        <div class="pagination-wrapper">
                          {!!
                              $applicant_21bis->appends(['search' => Request::get('search'),
