	...
	*/
    ...
    You could also use a data-* attribute and content for this. That way "bloats" the HTML, this way means you need to keep HTML and CSS in sync. Lea Verou has a clever way to handle with text-shadow.
		*/
		...
		td:nth-of-type(6):before { content: "วันที่สอบเทียบ"; }
...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
						...
						<div class="col-sm-12">
							<h3 class="pull-left">&nbsp;แจ้งผลการสอบเทียบเครื่องมือวัด</h3>
            ...
            <div class="col-sm-12">
                <div class="white-box">
                    ...
                    <div class="pull-right">
                      ...
                      @can('edit-'.str_slug('inform_calibration'))
                      ...
                      @can('add-'.str_slug('inform_calibration'))
                          <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/esurv/inform_calibration/create') }}">
                            <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                          </a>
                      @endcan

                      @can('delete-'.str_slug('inform_calibration'))
                    ...
                    <div class="table-responsive">
                      ...
                      {!! Form::open(['url' => '/esurv/inform_calibration/multiple', 'method' => 'delete', 'id' => 'myForm', 'class'=>'hide']) !!}

                      {!! Form::close() !!}

                      {!! Form::open(['url' => '/esurv/inform_calibration/update-state', 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                      ...
                      {!! Form::close() !!}
                        ...
                        <table class="table table-borderless" id="myTable">
                            ...
                            <tr>
                                ...
                                <th><input type="checkbox" id="checkall"></th>
																...
																<th>@sortablelink('calibration_date', 'วันที่สอบเทียบ')</th>
                            ...
                            <tbody>
                            @foreach($inform_calibration as $item)
                                <tr>
                                    ...
                                    <td class="text-top"><input type="checkbox" name="cb[]" class="cb" value="{{ $item->id }}"></td>
																		...
																		<td class="text-top">{{ HP::DateThai($item->calibration_date) }}</td>
																		<td class="text-top">{{ $item->applicant_name }}</td>
																		<td class="text-top">{{ $item->state=='1'?'ส่งข้อมูลแล้ว':'(ฉบับร่าง)' }}</td>
                                    <td class="text-top">
                                        @can('view-'.str_slug('inform_calibration'))
                                            <a href="{{ url('/esurv/inform_calibration/' . $item->id) }}"
                                               title="View inform_calibration" class="btn btn-info btn-xs">
                                                  <i class="fa fa-eye" aria-hidden="true"></i>
                                            </a>
                                        @endcan

                                        @can('edit-'.str_slug('inform_calibration'))
                                            <a href="{{ url('/esurv/inform_calibration/' . $item->id . '/edit') }}"
                                               title="Edit inform_calibration" class="btn btn-primary btn-xs">
                                        ...
                                        @can('delete-'.str_slug('inform_calibration'))
                                            {!! Form::open([
                                                            'method'=>'DELETE',
                                                            'url' => ['/esurv/inform_calibration', $item->id],
                                                            'style' => 'display:inline'
                                            ]) !!}
                                            {!! Form::button('<i class="fa fa-trash-o" aria-hidden="true"></i>', array(
                                                    'type' => 'submit',
                                                    'class' => 'btn btn-danger btn-xs',
                                                    'title' => 'Delete inform_calibration',
                        ...
                        <div class="pagination-wrapper">
                          {!!
                              $inform_calibration->appends(['search' => Request::get('search'),
