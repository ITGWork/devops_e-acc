...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">ระบบยื่นคำขอการทำผลิตภัณฑ์เพื่อใช้ในประเทศเป็นการเฉพาะคราว (20 ทวิ)</h3>

                    <div class="pull-right">

                      @can('add-'.str_slug('applicant-20bis'))
                          <a class="btn btn-success btn-sm waves-effect waves-light" href="{{ url('/esurv/applicant_20bis/create') }}">
                            <span class="btn-label"><i class="fa fa-plus"></i></span><b>เพิ่ม</b>
                          </a>
                      @endcan

                      @can('delete-'.str_slug('applicant-20bis'))
                    ...
                    <hr>
										...
										{!! Form::model($filter, ['url' => '/esurv/applicant_20bis', 'method' => 'get', 'id' => 'myFilter']) !!}
                    ...
                    <div class="table-responsive">
                      ...
                      {!! Form::open(['url' => '/esurv/applicant_20bis/multiple', 'method' => 'delete', 'id' => 'myForm', 'class'=>'hide']) !!}

                      {!! Form::close() !!}

                      {!! Form::open(['url' => '/esurv/applicant_20bis/update-state', 'method' => 'put', 'id' => 'myFormState', 'class'=>'hide']) !!}
                      ...
                      {!! Form::close() !!}
                        ...
                        <table class="table table-borderless" id="myTable">
                            ...
                            <tbody>
                            @foreach($applicant_20bis as $key => $item)
                                <tr>
                                    {{-- <td>{{ $applicant_20bis->firstItem()+$key }}</td> --}}
                                    <td class="text-top">{{ $applicant_20bis->perPage()*($applicant_20bis->currentPage()-1)+$loop->iteration }}</td>
                                    ...
                                    <td class="text-center">
                                        {{-- @can('view-'.str_slug('applicant-20bis'))
                                            <a href="{{ url('/esurv/applicant_20bis/' . $item->id) }}"
                                    ...
                                    @if(in_array($item->state,['1','3']))
                                           @can('edit-'.str_slug('applicant-20bis'))
                                                  <a href="{{ url('/esurv/applicant_20bis/' . $item->id . '/edit') }}"
                                         ...
                                         @can('delete-'.str_slug('applicant-20bis'))
                                                    {!! Form::open([
                                                                    'method'=>'DELETE',
                                                                    'url' => ['/esurv/applicant_20bis', $item->id],
                        ...
                        <div class="pagination-wrapper">
                          {!!
                              $applicant_20bis->appends(['search' => Request::get('search'),
