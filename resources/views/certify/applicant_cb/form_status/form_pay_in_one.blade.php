...
@section('content')
 ...
 {!! Form::open(['url' => 'certify/applicant-cb/pay-in/'.$pay_in->id,
                'class' => 'form-horizontal', 
                'files' => true,
                'id'=>"pay_in1_form"]) 
!!}
    <p>เรียน <span class="nameCertiLab">{{ @$pay_in->CertiCbCostTo->name ?? null }}</span> </p>
    <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ตามที่ท่านได้ยื่นคำขอรับใบรับรองหน่วยรับรอง 
        หมายเลขคำขอที่ <span class="noCertiLab" style="color:blue">{{  !empty($pay_in->CertiCbCostTo->app_no) ? $pay_in->CertiCbCostTo->app_no : null }}</span> 
   ...
   <div class="row">
    ...
    <div class="col-sm-8">
        <p>   {{ $pay_in->CertiCBAuditorsTo->auditor ?? null }} </p>
    </div>
</div>
        ...
        <div class="col-sm-4">
           ...
           @else 
                    @if (!is_null($pay_in->FileAttachPayInOne2To))
                        {{-- @if($pay_in->FileAttachPayInOne2To->file !='' && HP::checkFileStorage($attach_path.$pay_in->FileAttachPayInOne2To->file)) --}}
                            <a href="{{url('certify/check/file_cb_client/'.$pay_in->FileAttachPayInOne2To->file.'/'.( !empty($pay_in->FileAttachPayInOne2To->file_client_name) ? $pay_in->FileAttachPayInOne2To->file_client_name : 'null' ))}}" target="_blank">
