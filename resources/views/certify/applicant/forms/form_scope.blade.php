    ...
    <fieldset class="white-box">
        <legend>
            <h4>
                ...
                (<span class="text-warning">ห้องปฏิบัติการสอบเทียบ</span>) (Scope of Accreditation Sought  (<span class="text-warning">For calibration laboratory</span>)) 
        ...
        <div class="row form-group">
            ...
            <div class="col-sm-6">
                {!! Form::select('branch_lab_calibrate', [], null,  ['class' => 'form-control', 'id'=>'branch_lab_calibrate', 'placeholder' =>'- สาขาสอบเทียบ -']) !!}
                ...
                <div class="white-box m-t-20" style="border: 2px solid #e5ebec;">
                    <table class="table table-bordered" id="myTable_labCalibrate">
                        ...
                        <tbody id="labCalibrate_tbody">
                            ...
                            @if ( isset( $certi_lab->id ) && count( App\Models\Certify\Applicant\CertifyLabCalibrate::where('app_certi_lab_id', $certi_lab->id )->get() ) > 0 )
                                @php
                                    $certify_scope = App\Models\Certify\Applicant\CertifyLabCalibrate::where('app_certi_lab_id', $certi_lab->id )->get();
                                @endphp

                                @foreach ( $certify_scope as $kc => $itemC )
                                    @php
                                        $TableCalibrationBranch = $itemC->TableCalibrationBranch;
                                    @endphp
                                    <tr>
                                        <td class="text-center" style="vertical-align:top">{!! $kc+1 !!}</td>
                                        <td style="vertical-align:top">{!! !empty($TableCalibrationBranch->title )?$TableCalibrationBranch->title:null !!}</td>
                                        <td class="text-center">
                                            <input type="hidden" name="calibrate[branch_id][]" class="calibrate_branch_id" value="{!! $itemC->branch_id !!}">
                                            <button type="button" class="btn btn-danger btn-xs inTypeDeleteCalibrate" data-branch="{!! $itemC->branch_id !!}"  data-token="{!! $itemC->token !!}"><i class="fa fa-remove"></i></button>
                                        </td>
                                    </tr>
            ...
            <div class="col-md-12 box_section6" >
                <div class="form-group">
                    <div class="col-md-4  text-light">
                        {!! Form::label('attachs_sec62', 'กรุณาแนบไฟล์ขอบข่ายที่ต้องการยื่นขอการรับรอง', ['class' => 'col-md-12 label_attach text-light  control-label ']) !!}
                    </div>
                    @php
                        $section62_required = '';
                   @endphp
                    <div class="col-md-6" data-repeater-list="repeater-section62" >

                        @if ( isset( $certi_lab->id ) && count( App\Models\Certify\Applicant\CertiLabAttachAll::where('app_certi_lab_id', $certi_lab->id )->where('file_section', '62')->get() ) > 0 )

                            @php
                                    $section62_required = '';
                                $file_sectionn62 = App\Models\Certify\Applicant\CertiLabAttachAll::where('app_certi_lab_id', $certi_lab->id )->where('file_section', '62')->get();
                                
                            @endphp
          
                            @foreach ( $file_sectionn62 as $section62 )
                                <div class="row form-group">
                                    <div class="col-md-12">
                                        <a href="{!! HP::getFileStorage($attach_path.$section62->file) !!}" target="_blank" class="view_attach_sec62 btn btn-info btn-sm"> {!! HP::FileExtension($section62->file_client_name)  ?? '' !!}</a>
                                        <a href="{{url('certify/applicant/delete/file_app_certi_lab_attach_all').'/'.$section62->id.'/'.$certi_lab->token}}" class="btn btn-danger btn-xs box_remove_file" onclick="return confirm('ต้องการลบไฟล์นี้ใช่หรือไม่ ?')">
                            ...
                            <div class="col-md-10">
                                <div class="fileinput fileinput-new input-group" data-provides="fileinput">
                                    ...
                                    <span class="input-group-addon btn btn-default btn-file">
                                        ...
                                        <input type="file" name="attachs_sec62" class="attachs_sec62 check_max_size_file" {!! $section62_required !!}>
                                    </span>
                                    <a href="#" class="input-group-addon btn btn-default fileinput-exists" data-dismiss="fileinput">ลบ</a>
                                </div>
                            </div>
                            <div class="col-md-2">
                                <button class="btn btn-danger  delete-sec62" type="button" data-repeater-delete>
