...
@section('content')
 ...
 {!! Form::open(['url' => 'certify/applicant/update/status/auditor/'.$certi_lab->token,
 ...
 @foreach($certi_lab->certi_auditors_many as $key => $item)
...
<br>
@foreach($item->CertificateHistorys as $key1 => $log)
...
@if(isset($log->details_one))
<div class="row">
<div class="col-md-4 text-right">
   <p >ชื่อคณะผู้ตรวจประเมิน :</p>
</div>
<div class="col-md-8">
   <span>{{$log->details_one ?? '-'}}</span>
</div>
</div>
@endif
...
<div class="row">
    ...
    <div class="col-md-8">
        <a href="{{url('certify/check/file_client/'.$log->attachs.'/'.( !empty($log->attach_client_name) ? $log->attach_client_name : basename($log->attachs) ))}}" target="_blank">
            {!! HP::FileExtension($log->attachs)  ?? '' !!}
        </a>
    </div>
</div>
@endif
...
@if(!is_null($log->details_cost_confirm))
@php
...
<div class="col-md-12">
    <div class="form-group {{ $errors->has('no') ? 'has-error' : ''}}">
        {!! Form::label('no', 'กำหนดการตรวจประเมิน :', ['class' => 'col-md-4 control-label text-right']) !!}
        <div class="col-md-8">
...
@if(!is_null($log->remark))
<div class="col-md-12">
    <div class="form-group {{ $errors->has('no') ? 'has-error' : ''}}">
        {!! Form::label('no', 'หมายเหตุ :', ['class' => 'col-md-4 control-label text-right']) !!}
        <div class="col-md-8">
            {{ @$log->remark  ?? '-'}}
        </div>
    </div>
...
<div class="col-md-8">
    @foreach($attachs_file as $files)
            <p> 
                {{  @$files->file_desc  }}
                <a href="{{url('certify/check/file_client/'.$files->file.'/'.( !empty($files->file_client_name) ? $log->file_client_name :  basename($files->file) ))}}" target="_blank">
...
@if(!is_null($log->date))
<div class="col-md-12">
    <div class="form-group {{ $errors->has('no') ? 'has-error' : ''}}" target="_blank">
        {!! Form::label('no', 'วันที่บันทึก :', ['class' => 'col-md-4 control-label text-right']) !!}
        <div class="col-md-8">
...
@endforeach
 ...
 @if(count($certi_lab->certi_auditors_null_many) > 0)        
<div class="col-md-12">
    <div class="row">
        <div class="col-md-5 text-right">
            <p >วันที่บันทึก :</p>
